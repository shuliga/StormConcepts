package com.shl.poc.storm.api;

import com.shl.poc.storm.api.context.AppContext;
import com.shl.poc.storm.api.jms.MessageReceiver;
import com.shl.poc.storm.api.jms.MessageSender;
import com.shl.poc.strom.commons.event.command.CommandEvent;
import com.shl.poc.strom.commons.event.command.CommandJson;
import com.shl.poc.strom.commons.event.notification.NotificationEvent;
import com.shl.poc.strom.commons.event.notification.NotificationJson;
import com.shl.poc.strom.commons.security.Credentials;
import com.shl.poc.strom.commons.utils.FileUtil;
...
public class EventAPI {
	...
	private String registerAndGetToken(Credentials credentials) throws JMSException {
		...
		sender.sendCommand(new CommandEvent(CommandJson.Action.REGISTER.getLabel(), credentials.login, credentials.role, token, "EventAPI", null));
		return token;
	}
	...
	private void logoff(String token) throws JMSException {
		if (token != null) {
			...
			sender.sendCommand(new CommandEvent(CommandJson.Action.LOGOFF.getLabel(), null, null, token, "EventAPI", null));
		}
	}
